...
namespace PhysicsEngine
{
	...
	class CorridorWall : public DynamicActor
	{
	public:
		CorridorWall(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f) : DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x, dimensions.y, dimensions.z * 10), density);
			CreateShape(PxBoxGeometry(dimensions.x, dimensions.y, dimensions.z * 10), density);

			GetShape(0)->setLocalPose(PxTransform(PxVec3(0.f, .5f, .0f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(20.f, .5f, .0f)));
	...
	static PxVec3 pyramid_verts[] = { PxVec3(0,0,0), PxVec3(0,5,0), PxVec3(5,5,0), PxVec3(5,0,0), PxVec3(1.5f,0.0f,0.0f), PxVec3(1.5f,1.5f,0.0f), PxVec3(2.5f,1.5f,0.0f), PxVec3(2.5f,0.0f,0.0f) };
	static PxU32 pyramid_trigs[] = { 5,0,4, 5,0,1 ,6,1,5, 6,2,1, 2,6,3, 3,6,7};
	...
	class WindMillBuilding : public DynamicActor
	{
	public:
		WindMillBuilding(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f) : DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x/3, dimensions.y / 3, dimensions.z), density);	// left base
			CreateShape(PxBoxGeometry(dimensions.x/3, dimensions.y / 3, dimensions.z), density);	//right base
			CreateShape(PxBoxGeometry(dimensions.x, (dimensions.y / 3) * 2, dimensions.z), density);// body

			GetShape(0)->setLocalPose(PxTransform(PxVec3((dimensions.x / 3.0f) * -2, .0f, .0f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3((dimensions.x / 3.0f)*2, 0, 0)));
			GetShape(2)->setLocalPose(PxTransform(PxVec3(.0f, (dimensions.y / 3.0f) * 3, .0f)));
	...
	{
	public:
		WindMillBlades(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f) : DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(dimensions.x, dimensions.y / 2, dimensions.z * 2), density);
			CreateShape(PxBoxGeometry(dimensions.x, dimensions.y / 2, dimensions.z * 6.8), density);
			CreateShape(PxBoxGeometry(dimensions.x, dimensions.y / 2, dimensions.z * 6.8), density);

			//GetShape(0)->setLocalPose(PxTransform(PxVec3(.0f, .0f, .0f)));
			GetShape(0)->setLocalPose(PxTransform(PxQuat(PxPi / 2, PxVec3(0.f, 1.0f, .0f))));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(0.0f, .0f, .0f)));
			GetShape(2)->setLocalPose(PxTransform(PxQuat(PxPi / 2, PxVec3(1.f, .0f, 0.0f))));
	...
	{
	public:
		Club(const PxTransform& pose = PxTransform(PxIdentity), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = 1.f) : DynamicActor(pose)
		{
			CreateShape(PxBoxGeometry(1, .3, .3), density);
			CreateShape(PxBoxGeometry(.175, 4, .175), density);

			GetShape(0)->setLocalPose(PxTransform(PxVec3(0.6f, .3f, .0f)));
			GetShape(1)->setLocalPose(PxTransform(PxVec3(0.f, 4.5f, .0f)));
		}
	};
	...
	public:
		Trampoline(const PxTransform& pose = PxTransform(PxIdentity), const PxVec3& dimensions = PxVec3(1.f, 1.f, 1.f), PxReal stiffness = 1.f, PxReal damping = 1.f)
		{
			...
			bottom->GetShape(0)->setLocalPose(PxTransform(PxVec3(pose.p.x,bottom->GetShape()->getLocalPose().p.y , pose.p.z)));
			top->GetShape(0)->setLocalPose(PxTransform(PxVec3(pose.p.x, top->GetShape()->getLocalPose().p.y, pose.p.z)));
